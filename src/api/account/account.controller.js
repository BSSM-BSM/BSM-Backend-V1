...
const funcModel = require('../../service/function')
const crypto = require('crypto')
const sharp = require('sharp')
const mail = require('../../util/mail')
const pool = require('../../util/jwt')

let result
let dbResult
const login = async (req, res) =>{
    dbResult = await service.getMemberById(req.body.member_id)
    result={
        status:5,
        subStatus:0
    }
...
const signUp = async (req, res) =>{
    ...
    if(await service.signUp(req.body.member_id, req.body.member_pw, req.body.member_nickname, req.body.code)){
        result={
            status:1,
            subStatus:0
        }
...
const view = async (req, res) =>{
    ...
    dbResult = await service.getMemberByCode(memberCode)
    let member = {};
    if(dbResult){
...
const validCode = async (req, res) =>{
    dbResult = await service.getMemberFromCode(req.body.student_enrolled, req.body.student_grade, req.body.student_class, req.body.student_no, req.body.student_name)
    if(dbResult){
...
const pwEdit = async (req, res) =>{
    ...
    if(jwtValue.pwEdit||jwtValue.memberCode){
        ...
        if(req.body.member_pw!==req.body.member_pw_check){// 비밀번호 재입력 확인
            ...
            if(await service.pwEdit(memberCode, req.body.member_pw)){
                res.clearCookie('token');
                res.clearCookie('refreshToken');
                result={
                    status:1,
...
const token = async (req, res) => {
    ...
    rows = rows[0]
    // 유저 정보를 가져옴
    const dbResult = await service.getMemberByCode(rows.member_code);
    const payload = {
        isLogin:true,
        memberCode:dbResult.member_code,
        memberId:dbResult.member_id,
